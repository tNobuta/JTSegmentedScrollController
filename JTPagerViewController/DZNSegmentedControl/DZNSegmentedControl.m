...
- (CGRect)hairlineRect
{
    CGRect frame = CGRectMake(0.0f, 0.0f, self.frame.size.width, self.hairLineHeight);
...
- (void)setSelectedSegmentIndex:(NSInteger)segment
{
    ...
    [self setSelected:YES forSegmentAtIndex:segment movingIndicator:YES];
}

- (void)setSelectedSegmentIndexWithoutMovingIndicator:(NSInteger)segment {
    if (segment > self.numberOfSegments-1) {
        segment = 0;
    }
    
    [self setSelected:YES forSegmentAtIndex:segment movingIndicator:NO];
}
...
- (void)setSelected:(BOOL)selected forSegmentAtIndex:(NSUInteger)segment movingIndicator:(BOOL)movingIndicator
{
